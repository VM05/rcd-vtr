...
$(document).ready(function () {
  $('input[type="checkbox"]').click(function () {
    ...
    var cardsIniciales = $('.item-articulo').slice(0, 12);
    var categorias = $('input[type="checkbox"]:checked')
      .map(function () {
        return $(this).prop('id');
      })
      .get();
    if (categorias.length > 0) {
      // ocultar todas las cards y mostrar solo las que corresponden a las categorías seleccionadas
      $('.item-articulo')
        .hide()
        .filter('.' + categorias.join(',.'))
        .show();
      $('#pagination-container').hide();
    } else {
      // si no se ha seleccionado ninguna categoría, mostrar las primeras 12 cards
      $('.item-articulo').hide();
  ...
  var methods = {
    ...
    _draw: function () {
      ...
      var $panel =
        tagName === 'UL'
          ? this
          : $('<ul' + (o.listStyle ? ' class="' + o.listStyle + '"' : '') + '></ul>').appendTo(
              this
            );
    ...
    _ellipseClick: function ($panel) {
      ...
      $ellip.click(function (event) {
        if (!o.disable) {
          ...
          $this
            .html(
              '<input type="number" min="1" max="' + o.pages + '" step="1" value="' + val + '">'
            )
            .find('input')
            .focus()
